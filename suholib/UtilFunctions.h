...
namespace util
{
    static void ReplaceString(std::string &source, const std::string& from, const std::string& to)
    {
        size_t start_pos = 0;
    ...
    static void ToUpperString( const std::string& source, std::string& upperstring)
    {
        std::string upper(source);
        std::transform(upperstring.begin(), upperstring.end(), upperstring.begin(), ::toupper);
    }

    static void ToLowerString(const std::string& source, std::string& lowerstring)
    {
