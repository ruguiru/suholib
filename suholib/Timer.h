...
#define RECALC_INTERVAL		50			// 재계산 루프 횟수
    ...
    class Timer : public suho::pattern::singleton::Singleton<Timer>
    {
    public:
        enum TimerType
        {
    ...
    private:
        ...
        int CalcRemainMSec(const Timer::FlatTime timer_type);				                    // 다음 정각까지 남은 시간 계산 (밀리초)
    ...
    private:
        struct TimerInfo
        {
            ...
            TimerInfo() : _callback(nullptr), _interval_msec(0), _remain_msec(0), _timerType(INSTANT_TIMER), _arg(nullptr) {}
        ...
        std::mutex                                      _mutex;
        std::thread*                                    _timer_thread = nullptr;
    };
}   // end namespace suho
}   // end namespace time
