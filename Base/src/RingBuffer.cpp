...
bool RingBuffer::Peek( char* buf, int size )
{
	...
	else
	{
		...
		{
			std::size_t forward_space = static_cast<size_t>( size ) - static_cast<size_t>( space );

			memcpy( buf, _buffer.Begin() + _head, space );
			memcpy( buf + space, _buffer.Begin(), forward_space );
		}
	}
...
bool RingBuffer::Write( const char* data, int size )
{
	...
	if ( _head <= _tail )
	{
		...
		else
		{
			...
			memcpy( _buffer.Begin(), data + space, forward_space );
		}
	}
	else
	{
